...
annotation_dir=root_dir+"Ascending_Project_public/01_behavior_annotation_for_behavior_classifier/"
output_dir = root_dir+"Ascending_Project_public/output/Fig2_S4-GLM_jangles_legs_beh_DFF/"
...
def beh_dir(date, genotype, fly, trial):
    return root_dir+f"Ascending_Project_public/00_behavior_data_preprocess/df3dResults_ballRot_captureMeta/{date - 20000000}_{genotype}/Fly{fly}/CO2xzGG/behData_{trial:03d}/"


def sync_dir(date, genotype, fly, trial):
    Gal4=genotype.split('-')[0]
    return root_dir+f"Ascending_Project_public/03_general_2P_exp/{Gal4}/2P/{date}/{genotype}-fly{fly}/{genotype}-fly{fly}-sync-{trial:03d}"


def dir_2p(date, genotype, fly, trial):
    Gal4=genotype.split('-')[0]
    return root_dir+f"Ascending_Project_public/03_general_2P_exp/{Gal4}/2P/{date}/{genotype}-fly{fly}/{genotype}-fly{fly}-{trial:03d}"


def clc_output_dir(date, genotype, fly, trial):
    return root_dir+f"Ascending_Project_public/03_general_2P_exp/{genotype.split('-')[0]}/2P/{date}/{genotype}-fly{fly}/{genotype}-fly{fly}-{trial:03d}/output"
...
def get_PER_regressors(frame_counter, cam_line, date, genotype, fly, trial):
    ...
    pickle_file = root_dir+"Ascending_Project_public/00_behavior_data_preprocess/PE_regressors/"+Gal4+'/2P/'+date+'/'+genotype+'-'+fly+'/'+genotype+'-'+fly+'-'+trial+"/output/PER/camera_6/PER_labels_camera_6.p"
