...
# folder <- paste(root_dir,"Ascending_neuron_screen_analysis_pipeline/output/FigED4_ED5-overview_matrices_jangle_Rest/overview_angles_SN/", sep="")     
folder <- paste(root_dir,"Ascending_neuron_screen_analysis_pipeline/output/FigED4_ED5-overview_matrices_jangle_Rest/overview_angles_SN_rest/", sep="")                     
dir.create(folder)
intercept <- TRUE
standardize <- TRUE
...
for (path in Sys.glob(paste(root_dir,"Ascending_neuron_screen_analysis_pipeline/output/Fig2_S4-GLM_jangles_legs_beh_DFF/glm_input_files/[SRM]*.csv", sep=""))){
    ...
    ## Choose if constraining to Rest epochs:
    ## Contrain all behavior epochs
    # complete_df <- complete_df[complete_df$dFF_processing == "raw" & complete_df$Regressor_processing == "raw", ]

    ## Contrain the behavior data to "Rest" epochs only
    complete_df <- complete_df[complete_df$dFF_processing == "raw" & complete_df$Regressor_processing == "raw" & complete_df$Behaviour == "rest", ]
